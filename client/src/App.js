...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    API.getTasks().then((tasks) =>
      this.setState({ tasks: tasks, projects: this.getProjects(tasks) })
    );
  }
  ...
  filterTasks = (filter, project) => {
    switch (filter) {
      case "filter-important":
        API.getTasks("important").then((tasks) =>
          this.setState({ tasks: tasks, filter: "Important" })
        );
        break;
      case "filter-today":
        API.getTasks("today").then((tasks) =>
          this.setState({ tasks: tasks, filter: "Today" })
        );
        break;
      case "filter-week":
        API.getTasks("week").then((tasks) =>
          this.setState({ tasks: tasks, filter: "Next Week" })
        );
        break;
      case "filter-private":
        API.getTasks("private").then((tasks) =>
          this.setState({ tasks: tasks, filter: "Private" })
        );
        break;
      case "filter-shared":
        API.getTasks("shared").then((tasks) =>
          this.setState({ tasks: tasks, filter: "Shared" })
        );
        break;
      case "filter-project":
        API.getTasks(project).then((tasks) =>
          this.setState({ tasks: tasks, filter: project })
        );
        break;
      default:
        API.getTasks().then((tasks) =>
          this.setState({ tasks: tasks, filter: "All" })
        );
        break;
    }
  };
